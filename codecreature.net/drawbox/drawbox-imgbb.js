...
document.getElementById("submit").addEventListener("click", async function () {
  ...
  try {
    imageUploadRequest("drawing", imageData);
    ...
    alert("Error uploading image or submitting to imgBB.");
  } finally {
...
function imageUploadRequest(fileName, img) {
	const formObject = {
			key: 'fa7b55f9c42240f131430fe43eeb1175',
			name: fileName.split('.')[0],
			image: img
	};

	const formBody = Object.keys(formObject)
			.map(property => `${property}=${formObject[property]}`)
			.join("&");
