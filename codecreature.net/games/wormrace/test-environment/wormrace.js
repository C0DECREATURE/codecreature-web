...
function init() {
	...
	// for each key in the items table, put that item in the detailbox
	for ( let i in items ) {
...
function setHealth() {
	...
	var e = items.apple.movementEffect[w.health];
	...
	e = items.drink.movementEffect[w.health];
	if (typeof e != 'number') e = '?';
	else {
		e = Math.round( (e - 1) * 100 );
		if ( e >= 0 ) e = '+' + e;
...
function chooseItem(item) {
	...
	// set new chosen item
	itemChoice = item.id.replace("item-", "");
