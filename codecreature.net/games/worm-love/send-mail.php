...
if (!$mailboxOpen) {
	echo "Mailboxes are closed! Come back between February 1st and 9:00am February 14th.";
	...
	if (empty($response["error"])) {
		...
		if (!mysqli_query($worm_conn,$sql) ) {
			...
			if (empty($response["error"])) {
				...
				$response["typeText"] = "$emoji <strong>".$relationship_types[$type]["letter_name"]."</strong> $emoji";
				$response["newCount"] = $mail_received[$recipient][$type] + 1;
			}
		}
	}
