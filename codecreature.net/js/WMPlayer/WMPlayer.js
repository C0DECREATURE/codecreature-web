        ...
        if (typeof $config == 'string' || $config instanceof String || $config.nodeType !== undefined && $config.nodeType === 1 || window.jQuery && $config instanceof jQuery)
            ...
            if ($config.playlist !== undefined) {
                $config.playlist.forEach(function($audioTrack) {
                    self.addTrack($audioTrack.url, $audioTrack.title, $audioTrack.artist, $audioTrack.duration);
                });
            }
...
WMPlayer.prototype = {
    //add new audio track
    addTrack: function($url, $title, $artist, $duration) {
        if ($title === undefined) $title = "N/A";
        if ($artist === undefined) $artist = "N/A";
        if ($duration === undefined) $duration = "N/A";
        this.model.addAudioTrack($url, $title, $artist, $duration);
        return this;
    },
...
WMPlayer.prototype._Model.prototype = {
    ...
    getCurrentTrackArtist: function() {
        if(this.currentTrackIndex === null)
            ...
            return this.playlist[this.currentTrackIndex].artist;
    },
    ...
    //add audio track
    addAudioTrack: function($url, $title, $artist, $duration) {
        if($title === undefined)
            $title = 'N/A';
            ...
            if(playlists.length > 0) {
                for(var i = 0; i < $playlist.length; i++) {
                    ...
                    if($playlist[i].status == 'error')
                        status += ' wmp-error';
                    list = list+this.playlistPattern.replace('$index', (i+1)).replace('$title', $playlist[i].title).replace('$artist', $playlist[i].artist).replace('$duration', this.formatTime($playlist[i].duration)).replace('$status', status);
                }
            }
        }
