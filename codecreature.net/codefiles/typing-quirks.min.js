...
put this near the top of <head> element in a page that you want to run the script
		...
		<script src="/codefiles/typing-quirks.js?fileversion=9"></script>
...
tqCss.type = 'text/css';
tqCss.href = '/codefiles/typing-quirks.css?fileversion=5'; /* make sure this links to the location of the css file! */
document.querySelector('HEAD').appendChild(tqCss);

// assign text and alt text for all .tq elements in given parent element
// if no parent given, update for entire page
function tqAlts(parentEl) {
	if (!parentEl) parentEl = document;
	
	// format all unformatted .tq-format elements
	parentEl.querySelectorAll('.tq-format').forEach(tqFormatElement);
	...
	for ( let i = 0; i < parentEl.getElementsByClassName('tq').length; i++ ) {
		tq = parentEl.getElementsByClassName('tq')[i];
	...
	// TEXT EMOJIS
	for ( let i = 0; i < parentEl.getElementsByClassName('tq-e').length; i++ ) {
		tq = parentEl.getElementsByClassName('tq-e')[i];
		tq.classList.add('tq');
		...
		// alt text for tq
		alt = '';
		if (tq.dataset.a && tq.dataset.a != "false") { alt = tq.dataset.a; }
		else {
			if (txt==`:3` || txt==`X3` || txt==`:33` || txt==`X33`) alt = "smiling kitty";
			else if (txt==`:3c` || txt==`X3c` || txt==`:33c` || txt==`X33c`) alt = "mischievous kitty";
			else if (txt==`:'3` || txt==`:''3` || txt==`:'''3` || txt==`:'33` || txt==`:''33` || txt==`:'''33` ) alt = "crying kitty";
			if (txt==`;3` || txt==`;33`) alt = "winking kitty";
			else if (txt==`:)` || txt==`:))` || txt==`:)))`) alt = "smiley";
			else if (txt==`:(` || txt==`:((` || txt==`:(((`) alt = "frowny";
			else if (txt==`:'(` || txt==`:'((` || txt==`:'(((`) alt = "crying";
			else if (txt==`:')` || txt==`:'))` || txt==`:')))`) alt = "crying smiley";
			else if (txt==`B3` || txt==`B33`) alt = "sunglasses kitty";
			else if (txt==`B3c` || txt==`B33c`) alt = "mischievous sunglasses kitty";
			else if (txt==`B'3` || txt==`B''3` || txt==`B'''3` || txt==`B'33` || txt==`B''33` || txt==`B'''33`) alt = "crying sunglasses kitty";
			else if (txt==`:D` || txt==`:DD` || txt==`:DDD`) alt = "open mouth smiling";
			else if (txt==`XD` || txt==`XDD`) alt = "closed eyes laughing";
			else if (txt==`B)` || txt==`B))`) alt = "sunglasses";
			else if (txt==`;)` || txt==`;))` || txt==`;)))`) alt = "winking";
			else if (txt==`D:` || txt==`DD:` || txt==`DD:`) alt = "open mouth frowning";
			else if (txt==`:P`) alt = "tongue out";
			else if (txt==`XP`) alt = "closed eyes tongue out";
			
			if (alt == '') alt = "unknown emoticon";
			else alt += " face";
		}
		tq.name = alt;
	...
	for ( let i = 0; i < parentEl.getElementsByClassName('tq-nep').length; i++ ) {
		tq = parentEl.getElementsByClassName('tq-nep')[i];
		tq.classList.add('tq');
	...
	if (parentEl == document) {
		console.log('updated typing quirk alt texts for entire document. typing quirks visible = ' + tqOn);
	} else {
		console.log('set typing quirk alt texts for specified element.');
	}
}
...
function enableTq() {
	...
	updatePageSettingsDatabase("tqOn", tqOn);
	document.body.classList.add('tqOn');
	// reload
	tqAlts();
}
function disableTq() {
	...
	localStorage.setItem("tqOn", tqOn);
	updatePageSettingsDatabase("tqOn", tqOn);
	document.body.classList.remove('tqOn');
	// reload
	tqAlts();
}
