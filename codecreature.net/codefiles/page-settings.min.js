...
function updatePageSettingsDatabase(setting,value) {
...
function makePageSettingsElement(psParent) {
	...
	if (!pageSettingsContents) {
		...
		title.innerHTML = 'site-wide settings';
		header.appendChild(title);
		pageSettingsContents.appendChild(header);
...
function addCustomCursorToggle() {
	...
	addPageSettingCheckbox(cursorOnCheckbox,`default cursor`,`check to disable custom cursors`);
	...
	if (localStorage.getItem("customCursorOn")) {
		...
		// apply
		if (isTrue(customCursorOn)) { setCustomCursor(true); }
		else { setCustomCursor(false); }
	}
	// functions to enable/disable custom scrollbars
	function toggleCustomCursor() {
		setCustomCursor(!customCursorOn);
	}
	function setCustomCursor(bool) {
		customCursorOn = isTrue(bool);
		localStorage.setItem("customCursorOn", customCursorOn);
		updatePageSettingsDatabase("customCursorOn",customCursorOn);
		updateCustomCursorCheckbox();
		if (customCursorOn) {
...
/******************************************************************************/
// CUSTOM SCROLLBARS TOGGLE

function addCustomScrollbarsToggle() {
	var customScrollbarsOn = true;
	var customScrollbarCheckbox = document.createElement('input');
	
	customScrollbarCheckbox.id = "scrollbarsOnCheckbox";
	customScrollbarCheckbox.addEventListener('click', toggleCustomScrollbar);
	addPageSettingCheckbox(customScrollbarCheckbox,`default scrollbars`,`check to disable custom js scrollbars. page refresh required`);
	// if there is already a custom scrollbars setting in local storage, apply it
	if (localStorage.getItem("customScrollbarsOn")) {
		customScrollbarsOn = localStorage.getItem("customScrollbarsOn");
		updateCustomScrollbarCheckbox();
		// apply
		if (isTrue(customScrollbarsOn)) { setCustomScrollbar(true); }
		else { setCustomScrollbar(false); }
	}
	// functions to enable/disable custom scrollbars
	function toggleCustomScrollbar() {
		setCustomScrollbar(!customScrollbarsOn);
	}
	function setCustomScrollbar(bool) {
		customScrollbarsOn = isTrue(bool);
		localStorage.setItem("customScrollbarsOn", customScrollbarsOn);
		updatePageSettingsDatabase("customScrollbarsOn",customScrollbarsOn);
		updateCustomScrollbarCheckbox();
		if (customScrollbarsOn) {
	...
	function updateCustomScrollbarCheckbox() { customScrollbarCheckbox.checked = !isTrue(customScrollbarsOn); }
}